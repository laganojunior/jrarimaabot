...
//////////////////////////////////////////////////////////////////////////////
//Returns a static evaluation score of the current board position in the
//perspective of the player given
//////////////////////////////////////////////////////////////////////////////
short Board :: eval(unsigned char color)
{
    ...
    //material
    int material = __builtin_popcountll(pieces[GOLD][ELEPHANT]) * 2000
                 + __builtin_popcountll(pieces[GOLD][CAMEL]) * 700 
                 + __builtin_popcountll(pieces[GOLD][HORSE]) * 500 
                 + __builtin_popcountll(pieces[GOLD][DOG]) * 300 
                 + __builtin_popcountll(pieces[GOLD][CAT]) * 200
                 + __builtin_popcountll(pieces[GOLD][RABBIT]) * 100
                 - __builtin_popcountll(pieces[SILVER][ELEPHANT]) * 2000
                 - __builtin_popcountll(pieces[SILVER][CAMEL]) * 700 
                 - __builtin_popcountll(pieces[SILVER][HORSE]) * 500 
                 - __builtin_popcountll(pieces[SILVER][DOG]) * 300 
                 - __builtin_popcountll(pieces[SILVER][CAT]) * 200
                 - __builtin_popcountll(pieces[SILVER][RABBIT]) * 100;

    //rabbit advancement
    int advance = __builtin_popcountll(pieces[GOLD][RABBIT] & getRow(0)) * 30000
                + __builtin_popcountll(pieces[GOLD][RABBIT] & getRow(1)) * 100
                + __builtin_popcountll(pieces[GOLD][RABBIT] & getRow(2)) * 10
                + __builtin_popcountll(pieces[GOLD][RABBIT] & getRow(3)) * 9
                + __builtin_popcountll(pieces[GOLD][RABBIT] & getRow(4)) * 8
                + __builtin_popcountll(pieces[GOLD][RABBIT] & getRow(5)) * 7
                + __builtin_popcountll(pieces[GOLD][RABBIT] & getRow(6)) * 6
                + __builtin_popcountll(pieces[GOLD][RABBIT] & getRow(7)) * 50
                - __builtin_popcountll(pieces[SILVER][RABBIT] & getRow(7)) * 30000
                - __builtin_popcountll(pieces[SILVER][RABBIT] & getRow(6)) * 100
                - __builtin_popcountll(pieces[SILVER][RABBIT] & getRow(5)) * 10
                - __builtin_popcountll(pieces[SILVER][RABBIT] & getRow(4)) * 9
                - __builtin_popcountll(pieces[SILVER][RABBIT] & getRow(3)) * 8
                - __builtin_popcountll(pieces[SILVER][RABBIT] & getRow(2)) * 7
                - __builtin_popcountll(pieces[SILVER][RABBIT] & getRow(1)) * 6
                - __builtin_popcountll(pieces[SILVER][RABBIT] & getRow(0)) * 50;

    //keeps center control with the strong pieces
    int strongCenter = __builtin_popcountll(pieces[GOLD][ELEPHANT] & getCenterRing(0)) * 50
                     + __builtin_popcountll(pieces[GOLD][ELEPHANT] & getCenterRing(1)) * 40
                     + __builtin_popcountll(pieces[GOLD][ELEPHANT] & getCenterRing(2)) * 30
                     + __builtin_popcountll(pieces[GOLD][ELEPHANT] & getCenterRing(3)) * 20
                     + __builtin_popcountll(pieces[GOLD][CAMEL] & getCenterRing(0)) * 30
                     + __builtin_popcountll(pieces[GOLD][CAMEL] & getCenterRing(1)) * 20
                     + __builtin_popcountll(pieces[GOLD][CAMEL] & getCenterRing(2)) * -50
                     + __builtin_popcountll(pieces[GOLD][CAMEL] & getCenterRing(3)) * -100
                     + __builtin_popcountll(pieces[GOLD][HORSE] & getCenterRing(0)) * 40
                     + __builtin_popcountll(pieces[GOLD][HORSE] & getCenterRing(1)) * 30
                     + __builtin_popcountll(pieces[GOLD][HORSE] & getCenterRing(2)) * 20
                     + __builtin_popcountll(pieces[GOLD][HORSE] & getCenterRing(3)) * 30
                     - __builtin_popcountll(pieces[SILVER][ELEPHANT] & getCenterRing(0)) * 50
                     - __builtin_popcountll(pieces[SILVER][ELEPHANT] & getCenterRing(1)) * 40
                     - __builtin_popcountll(pieces[SILVER][ELEPHANT] & getCenterRing(2)) * 30
                     - __builtin_popcountll(pieces[SILVER][ELEPHANT] & getCenterRing(3)) * 20
                     - __builtin_popcountll(pieces[SILVER][CAMEL] & getCenterRing(0)) * 40
                     - __builtin_popcountll(pieces[SILVER][CAMEL] & getCenterRing(1)) * 20
                     - __builtin_popcountll(pieces[SILVER][CAMEL] & getCenterRing(2)) * -50
                     - __builtin_popcountll(pieces[SILVER][CAMEL] & getCenterRing(3)) * -100
                     - __builtin_popcountll(pieces[SILVER][HORSE] & getCenterRing(0)) * 40
                     - __builtin_popcountll(pieces[SILVER][HORSE] & getCenterRing(1)) * 30
                     - __builtin_popcountll(pieces[SILVER][HORSE] & getCenterRing(2)) * 20
                     - __builtin_popcountll(pieces[SILVER][HORSE] & getCenterRing(3)) * 30;

    //keep rabbits away from center 
    int rabbitCenterAvoid = __builtin_popcountll(pieces[GOLD][RABBIT] & getCenterRing(0)) * -100
                          + __builtin_popcountll(pieces[GOLD][RABBIT] & getCenterRing(1)) * -75
                          + __builtin_popcountll(pieces[GOLD][RABBIT] & getCenterRing(2)) * -25
                          + __builtin_popcountll(pieces[GOLD][RABBIT] & getCenterRing(3)) * 25;
                          - __builtin_popcountll(pieces[SILVER][RABBIT] & getCenterRing(0)) * -100
                          - __builtin_popcountll(pieces[SILVER][RABBIT] & getCenterRing(1)) * -75
                          - __builtin_popcountll(pieces[SILVER][RABBIT] & getCenterRing(2)) * -25
                          - __builtin_popcountll(pieces[SILVER][RABBIT] & getCenterRing(3)) * 25;

    //keep pieces away from traps
    int trapAvoid = __builtin_popcountll(pieces[GOLD][CAMEL] & (getTraps() | getTrapNeighbors()))    * -300
                  + __builtin_popcountll(pieces[GOLD][HORSE] & (getTraps() | getTrapNeighbors()))    * -100
                  + __builtin_popcountll(pieces[GOLD][DOG] & (getTraps() | getTrapNeighbors()))      * -50
                  + __builtin_popcountll(pieces[GOLD][CAT] & (getTraps() | getTrapNeighbors()))      * -25
                  + __builtin_popcountll(pieces[GOLD][RABBIT] & (getTraps() | getTrapNeighbors()))   * -50
                  - __builtin_popcountll(pieces[SILVER][CAMEL] & (getTraps() | getTrapNeighbors()))  * -300 
                  - __builtin_popcountll(pieces[SILVER][HORSE] & (getTraps() | getTrapNeighbors()))  * -100
                  - __builtin_popcountll(pieces[SILVER][DOG] & (getTraps() | getTrapNeighbors()))    * -50
                  - __builtin_popcountll(pieces[SILVER][CAT] & (getTraps() | getTrapNeighbors()))    * -25
                  - __builtin_popcountll(pieces[SILVER][RABBIT] & (getTraps() | getTrapNeighbors())) * -50;

    //guard local traps with weaker pieces
    int trapGuard = __builtin_popcountll(pieces[GOLD][DOG] & (getTrapNeighbors()) & 
                      (getRow(5) | getRow(6) | getRow(7) )) * 100
                  + __builtin_popcountll(pieces[GOLD][CAT] & (getTrapNeighbors()) & 
                      (getRow(5) | getRow(6) | getRow(7) )) * 75
                  - __builtin_popcountll(pieces[SILVER][DOG] & (getTrapNeighbors()) & 
                      (getRow(1) | getRow(2) | getRow(3) )) * 100
                  - __builtin_popcountll(pieces[SILVER][CAT] & (getTrapNeighbors()) & 
...
//////////////////////////////////////////////////////////////////////////////
bool Board :: isWin(unsigned char color)
{
    if (color == GOLD)
        return pieces[GOLD][RABBIT] & getRow(0);
    else
        return pieces[SILVER][RABBIT] & getRow(7);
}
